...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `stock_prices`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in stock_prices.__all__ or not hasattr(stock_prices, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in stock_prices.__all__:
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("stock_prices")
    for item in inventory.values():
        if (
            item.domain == "py"
            and "(" not in item.name
            and (item.name == "stock_prices" or item.name.startswith("stock_prices."))
        ):
