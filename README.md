...
Бот использует тот же pipeline, что CLI: загружает данные, готовит датасет, рендерит MP4 и отправляет ролик обратно в чат. Команда `/ideas` показывает готовые сценарии для Пульса с кнопками запуска.
