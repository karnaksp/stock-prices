...
Бот использует тот же pipeline, что CLI: загружает данные, готовит датасет, рендерит MP4 и отправляет ролик обратно в чат. Команда `/ideas` показывает готовые сценарии для Пульса с кнопками запуска. После preset-сценария бот отправляет не только видео, но и готовый текст поста: хук, описание, хэштеги и музыкальное/монтажное настроение. После текста появляются follow-up кнопки: быстрый черновик 4s, полноценный шортс 16s и вариант 12s.
