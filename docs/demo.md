...
Telegram-бот рассчитан на короткие запросы без знания CLI. Пользователь пишет тикер или несколько параметров в одном сообщении, бот отвечает статусом, генерирует видео и отправляет MP4. Команды `/menu` и `/меню` открывают единое кнопочное меню для идей, примеров, draft-прогонов, случайного draft и очереди.
