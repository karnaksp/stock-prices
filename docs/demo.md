...
Telegram-бот рассчитан на короткие запросы без знания CLI. Пользователь пишет тикер или несколько параметров в одном сообщении, бот отвечает статусом, генерирует видео и отправляет MP4. Команды `/start`, `/menu` и `/меню` открывают единое кнопочное меню для быстрого запуска, шпаргалки производства, помощи по синтаксису, прямого запуска топовых shorts-сценариев, полного пакета shorts, идей, примеров, пакета для Пульса, контент-плана, недели публикаций, публикации дня, поста дня, публикационных пакетов preset-сценариев, draft-прогонов, случайного draft и очереди. Команды `/quick` и `быстрый запуск` открывают компактный пульт: снять день, снять неделю, top-серия Studio, случайный шортс, контент-план, посты недели и очередь. Команды `/guide`, `/workflow` и `шпаргалка` показывают короткий путь без рендера: публикация дня, пост дня и проверка очереди. Команды `/publish_week` и `снять неделю` ставят все 7 shorts недельного плана в очередь и отправляют чеклист публикаций. Команды `/week_posts` и `посты недели` отправляют все 7 готовых Pulse-текстов недельного плана отдельными сообщениями без рендера. Команды `/publish_day` и `снять день` ставят шортс дня в очередь и сразу отправляют пакет для поста. Команды `/today_post` и `пост дня` открывают готовый текст текущего дня без рендера. Команды `/shorts SBER LKOH за год` и `/draft metals` добавляют нужный режим к обычному запросу и ставят задачу в очередь.

Для контента в Пульсе можно запускать готовые preset-сценарии. Команды `top drafts` и `топ черновики` ставят в очередь быстрые draft-прогоны только top-shorts сценариев из меню. Команды `top shorts` и `топ шортсы` ставят эти же сценарии в очередь как полноценные shorts-ролики. Команды `top shorts studio`, `top shorts aurora` и `топ шортсы студио` ставят ту же серию сразу в выбранной теме. Команды `все шортсы`, `/all_shorts` и `all shorts` ставят в очередь полноценные shorts-ролики по всей preset-библиотеке, а `все шортсы студио`, `все шортсы аурора` и `all shorts aurora` делают такой же полный выпуск в едином стиле. Команды `variants metals` и `варианты металлы` ставят один preset сразу в `default`, `aurora` и `studio`, чтобы быстро выбрать лучший визуал. Команды `/kits` и `пакеты` открывают кнопочный список публикационных пакетов всех preset-сценариев без рендера. Команды `kit metals` и `пакет металлы` открывают публикационный пакет одного preset без немедленного рендера: хук, обложки, музыку и кнопки запуска. Команды `/pack` и `пакет пульса` показывают компактный top-пакет: для каждого сценария сразу есть команда шортса, черновика и поста без рендера, а также кнопки `Top Studio`, `Top Aurora`, `Все шортсы`, `Все Studio` и `Все Aurora`. Команды `/plan`, `план пульса` и `контент-план` показывают недельную сетку готовых выпусков с командами, обложками, музыкой и кнопками запуска конкретного дня; `/publish_week` и `снять неделю` ставят все 7 shorts этой сетки в очередь и присылают чеклист публикаций, `/week_posts` и `посты недели` отправляют все 7 готовых текстов без рендера, `/plan_shorts` и `снять план` ставят те же shorts без чеклиста, `/publish_day` и `снять день` ставят шортс дня в очередь и сразу отправляют пакет для поста, `/today` и `шортс дня` ставят только один выпуск текущего дня, а `/today_post` и `пост дня` открывают готовый текст текущего дня без рендера. Команды `случайный шортс`, `random shorts` и `/random_shorts` выбирают один проверенный preset и сразу ставят полноценный shorts-ролик в очередь. Команды `случайный черновик`, `random draft` и `/random_draft` делают то же в коротком draft-режиме без LLM. Команды `/examples` и `/примеры` показывают кнопки проверенных запросов по рынкам: металлы, российские акции, иностранные акции, крипта, фьючерсы и валюта. Команды `черновики примеров` и `случайный пример` помогают быстро прогнать эти проверенные запросы в draft-режиме.

После каждого MP4 бот присылает черновик поста для Пульса. Для готовых сценариев это ручной текст, для остальных запросов - skeleton с заголовком, хуком, фактическими параметрами, вопросом для обсуждения и монтажным настроением. Если нужен только текст до рендера, команды `post metals`, `post LKOH SBER 2020 2024` и `текст SBER LKOH за год` возвращают такой пакет без постановки задачи в очередь, а `/posts` и `посты` открывают кнопки всех preset-пакетов. После preset- и custom-видео бот показывает кнопки для повторного запуска того же запроса как draft 4s, shorts 16s, вариант 12s или быстрый визуальный вариант Aurora/Studio; после preset-видео есть кнопка `Все темы` для трех визуальных версий.

Команды `/today_post` и `пост дня` открывают готовый текст текущего дня без рендера. Команды `/today_kit` и `пакет дня` открывают полный публикационный пакет текущего дня из контент-плана без рендера: хук, варианты обложки, музыкальные референсы, пост и кнопки запуска.
...
каждый месяц 30к       короткая запись monthly=30000
по 30 тыс рублей в месяц короткая запись monthly=30000 и RUB
