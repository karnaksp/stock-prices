...
Для контента в Пульсе можно запускать готовые preset-сценарии. Команды `top drafts` и `топ черновики` ставят в очередь быстрые draft-прогоны только top-shorts сценариев из меню. Команды `top shorts` и `топ шортсы` ставят эти же сценарии в очередь как полноценные shorts-ролики. Команды `/pack` и `пакет пульса` показывают компактный top-пакет: для каждого сценария сразу есть команда шортса, черновика и поста без рендера. Команды `случайный черновик`, `random draft` и `/random_draft` выбирают один проверенный preset и ставят короткий draft в очередь без LLM. Команды `/examples` и `/примеры` показывают кнопки проверенных запросов по рынкам: металлы, российские акции, иностранные акции, крипта, фьючерсы и валюта. Команды `черновики примеров` и `случайный пример` помогают быстро прогнать эти проверенные запросы в draft-режиме.
