...
Telegram-бот рассчитан на короткие запросы без знания CLI. `/start`, `/menu`, `/shoot` и `снять` открывают компактный пульт из шести действий, а `/help` показывает короткую справку в один экран.
...
Для шортса без ручного выбора напишите `/shorts`: бот покажет готовые истории. Для своего ролика используйте `/shorts SBER LKOH за год`. Длинный рендер может идти несколько минут; очередь смотрите через `/queue`, `статус` или кнопку `Очередь`.
