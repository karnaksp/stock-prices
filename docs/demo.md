...
Telegram-бот рассчитан на короткие запросы без знания CLI. Пользователь пишет тикер или несколько параметров в одном сообщении, бот отвечает статусом, генерирует видео и отправляет MP4. Команды `/start`, `/menu` и `/меню` открывают единое кнопочное меню для помощи по синтаксису, прямого запуска топовых shorts-сценариев, полного пакета shorts, идей, примеров, пакета для Пульса, контент-плана, шортса дня, публикационных пакетов preset-сценариев, draft-прогонов, случайного draft и очереди. Команды `/shorts SBER LKOH за год` и `/draft metals` добавляют нужный режим к обычному запросу и ставят задачу в очередь.
