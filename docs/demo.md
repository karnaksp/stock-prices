...
Telegram-бот рассчитан на короткие запросы без знания CLI. Пользователь пишет тикер или несколько параметров в одном сообщении, бот отвечает статусом, генерирует видео и отправляет MP4. Команды `/menu` и `/меню` открывают единое кнопочное меню для помощи по синтаксису, прямого запуска топовых shorts-сценариев, идей, примеров, draft-прогонов, случайного draft и очереди.

Для контента в Пульсе можно запускать готовые preset-сценарии. Команды `top drafts` и `топ черновики` ставят в очередь быстрые draft-прогоны только top-shorts сценариев из меню. Команды `top shorts` и `топ шортсы` ставят эти же сценарии в очередь как полноценные shorts-ролики. Команды `случайный черновик`, `random draft` и `/random_draft` выбирают один проверенный preset и ставят короткий draft в очередь без LLM. Команды `/examples` и `/примеры` показывают кнопки проверенных запросов по рынкам: металлы, российские акции, иностранные акции, крипта, фьючерсы и валюта. Команды `черновики примеров` и `случайный пример` помогают быстро прогнать эти проверенные запросы в draft-режиме.
