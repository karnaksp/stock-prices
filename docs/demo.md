...
Для контента в Пульсе можно запускать готовые preset-сценарии. Команды `случайный черновик`, `random draft` и `/random_draft` выбирают один проверенный preset и ставят короткий draft в очередь без LLM. Команды `/examples` и `/примеры` показывают кнопки проверенных запросов по рынкам: металлы, российские акции, иностранные акции, крипта, фьючерсы и валюта. Команды `черновики примеров` и `случайный пример` помогают быстро прогнать эти проверенные запросы в draft-режиме.

После каждого MP4 бот присылает черновик поста для Пульса. Для готовых сценариев это ручной текст, для остальных запросов - skeleton с заголовком, хуком, фактическими параметрами, вопросом для обсуждения и монтажным настроением. После preset- и custom-видео бот показывает кнопки для повторного запуска того же запроса как draft 4s, shorts 16s или вариант 12s.

Когда запущено несколько роликов подряд, кнопка `Статус очереди`, команда `/queue` или `очередь` показывает активный job, короткий текст запроса и список ожидающих задач.
...
Пачка роликов одним сообщением:
