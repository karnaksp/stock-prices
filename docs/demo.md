...
Готовые preset-сценарии и публикационные пакеты заранее описаны в проекте: бот не использует LLM и не автогенерирует идеи. Подробные команды лежат в разделе [Telegram-запросы](telegram.md).
