...
Telegram-бот рассчитан на короткие запросы без знания CLI. Пользователь пишет тикер или несколько параметров в одном сообщении, бот отвечает статусом, генерирует видео и отправляет MP4. Команды `/start`, `/menu` и `/меню` открывают единое кнопочное меню для шпаргалки производства, помощи по синтаксису, прямого запуска топовых shorts-сценариев, полного пакета shorts, идей, примеров, пакета для Пульса, контент-плана, недели публикаций, публикации дня, поста дня, публикационных пакетов preset-сценариев, draft-прогонов, случайного draft и очереди. Команды `/guide`, `/workflow` и `шпаргалка` показывают короткий путь без рендера: публикация дня, пост дня и проверка очереди. Команды `/publish_week` и `снять неделю` ставят все 7 shorts недельного плана в очередь и отправляют чеклист публикаций. Команды `/week_posts` и `посты недели` отправляют все 7 готовых Pulse-текстов недельного плана отдельными сообщениями без рендера. Команды `/publish_day` и `снять день` ставят шортс дня в очередь и сразу отправляют пакет для поста. Команды `/today_post` и `пост дня` открывают готовый текст текущего дня без рендера. Команды `/shorts SBER LKOH за год` и `/draft metals` добавляют нужный режим к обычному запросу и ставят задачу в очередь.
