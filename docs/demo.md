...
Команды `/today_kit` и `пакет дня` открывают публикационный пакет текущего дня из контент-плана без рендера: хук, варианты обложки, музыкальные референсы, пост и кнопки запуска.
