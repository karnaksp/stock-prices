...
Telegram-бот рассчитан на короткие запросы без знания CLI. `/start`, `/menu` и `/меню` открывают компактное меню, `/shoot` и `снять` открывают быстрый пульт, а `/help` показывает короткую справку в один экран.
...
Для контента в Пульсе есть фиксированные preset-сценарии, публикационные пакеты и готовые тексты без LLM и автопридумывания идей. Подробные команды лежат в разделе [Telegram-запросы](telegram.md).

`/publish_week` собирает сбалансированную неделю: новая экономика, металлы, банки, связь, ритейл, энергетика и голубые фишки.

Новые спокойные российские сюжеты для менее хайпового контента запускаются коротко:
...
4. Открыть Telegram-бота и отправить `/shoot`, затем `gold silver palladium 2010-2026 RUB capital invest initial=0 monthly=30000 gradient`.
5. Показать ответ бота: статус генерации, готовый MP4 и текст для Пульса.
...
- Очередь задач для Telegram и стабильный Docker-режим.
