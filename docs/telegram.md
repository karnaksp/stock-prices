...
Чтобы проверить, чем занят бот, отправьте `/queue`, `/status` или `очередь`. Ответ показывает активный job, список ожидающих job id и счетчики завершенных/ошибочных задач. Это удобно после команды `все черновики` или нескольких длинных shorts подряд.
