...
Самый простой вход - `/menu` или `/меню`. Бот покажет кнопки для основных действий: прямой запуск топовых shorts-сценариев, готовые идеи, проверенные примеры, draft-presets, draft-примеры, случайный preset, случайный пример и статус очереди. Это удобно, когда не хочется вспоминать точные команды.
