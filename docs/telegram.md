...
При старте бот также настраивает постоянную кнопку `Mini App` в меню Telegram. Если кнопка меню недоступна в клиенте, команда `/app` пришлет Telegram-кнопку "Открыть Mini App". Внутри можно выбрать тикеры, рынок, период, валюту, режим ролика, тему, инвестиционный сценарий, `duration`, `fps`, `gradient` и `legend`. После отправки приложение передает готовую строку запроса в бота, а бот ставит задачу в очередь.
...
URL можно переопределить через переменную окружения `STOCK_PRICES_MINI_APP_URL`. Постоянная кнопка меню включена по умолчанию; для отключения используйте `STOCK_PRICES_MINI_APP_MENU_BUTTON=false` или CLI-флаг `--no-mini_app_menu_button`.
