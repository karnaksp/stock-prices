...
После постановки задачи бот показывает кнопку `Статус очереди`. Нажмите ее, чтобы обновить snapshot без нового текстового запроса. То же самое можно сделать командами `/queue`, `/status` или `очередь`: ответ показывает активный job, список ожидающих job id и счетчики завершенных/ошибочных задач. Это удобно после команды `все черновики`, multi-line сообщения или нескольких длинных shorts подряд.
