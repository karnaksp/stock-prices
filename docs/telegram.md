...
Для быстрых шортсов можно не вспоминать тикеры и даты, а взять проверенный сценарий. Отправьте `/ideas`, и бот покажет список вместе с кнопками. Нажатие на кнопку сразу ставит выбранный сценарий в очередь.

После MP4 по preset-сценарию бот дополнительно отправляет текст для Пульса: хук, короткое описание, хэштеги, музыкальное/монтажное настроение и дисклеймер.
