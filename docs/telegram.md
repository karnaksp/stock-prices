...
После MP4 бот дополнительно отправляет текст для Пульса. Для preset-сценариев это ручной хук, короткое описание, хэштеги, музыкальное/монтажное настроение и дисклеймер. Для custom-запросов это нейтральный шаблон по фактическим параметрам: тикеры, период, валюта, метрика и инвестиционный сценарий. Следующим сообщением после preset-сценария бот показывает follow-up кнопки для этого же сценария: быстрый черновик 4s, полноценный шортс 16s и вариант 12s.
