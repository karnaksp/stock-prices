...
Самый простой вход - `/menu` или `/меню`. Бот покажет кнопки для основных действий: готовые идеи, проверенные примеры, draft-presets, draft-примеры, случайный preset, случайный пример и статус очереди. Это удобно, когда не хочется вспоминать точные команды.
