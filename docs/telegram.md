...
Бот пришлет Telegram-кнопку "Открыть Mini App". Внутри можно выбрать тикеры, рынок, период, валюту, режим ролика, тему, инвестиционный сценарий, `duration`, `fps`, `gradient` и `legend`. После отправки приложение передает готовую строку запроса в бота, а бот ставит задачу в очередь.
...
URL можно переопределить через переменную окружения `STOCK_PRICES_MINI_APP_URL`.
