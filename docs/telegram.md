...
Запрос можно писать сухо или обычной фразой. Слова вроде `сделай`, `покажи`, `про`, `и`, `для Пульса`, `график`, `видео` игнорируются как служебные, поэтому `сделай шортс про SBER и LKOH за 6 месяцев для Пульса` работает так же, как `SBER LKOH за 6 месяцев shorts`. Предлог `с` можно использовать для сравнения тикеров: `сравни SBER с LKOH за 6 месяцев шортс`.
