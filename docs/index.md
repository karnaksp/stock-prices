...
/guide, /workflow и шпаргалка открывают короткий production-workflow без рендера: пакет дня -> шортс дня -> пост/обложка/музыка -> очередь
/pack показывает top-сценарии сразу в трех форматах: шортс, черновик, пост без рендера
/plan, план пульса и контент-план показывают недельную сетку готовых выпусков с кнопками запуска, /plan_shorts и снять план ставят все shorts этой сетки в очередь, /today и шортс дня ставят выпуск текущего дня, а /today_kit и пакет дня открывают пакет текущего дня без рендера
...
кнопки /menu открывают шпаргалку, помощь, top-shorts сценарии, идеи, примеры, draft-прогоны и очередь
