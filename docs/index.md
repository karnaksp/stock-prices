...
Для выбора сюжета под настроение поста используйте `категории`: тихие российские истории, драмы и просадки, сырьевые циклы, рост и недельный план. Для быстрого запуска работают команды вроде `top quiet`, `random drama` и `черновик тихие`.
