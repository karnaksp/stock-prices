...
=== "Telegram"
    ...
    Бот не использует LLM: `/shorts` открывает ручные preset, `случайный шортс` собирает подборку из тикерной вселенной, а запрос с тикерами ставит в очередь конкретный ролик.
...
| Случайная подборка | `случайный шортс` |
