...
`/start`, `/menu` и `/меню` открывают компактный пульт. Первый экран оставляет только частые действия: снять ролик, день, неделю, истории, очередь и короткую справку.

Для ежедневной работы быстрее писать `/shoot` или `снять`: бот покажет пульт с действиями `Снять день`, `Снять неделю`, `Top Studio`, `Случайный`, `Очередь` и `Меню`.
